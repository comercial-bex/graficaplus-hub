            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Esta ação não pode ser desfeita. OS, orçamentos e pagamentos vinculados podem
                    ser afetados.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, vendedor_id: v === "none" ? null : v })}
              >
                <SelectTrigger>
    ...
    <Card>
      <CardContent className="p-6 grid md:grid-cols-2 gap-4 text-sm">
        ...
        <div>
          ...
          <div>{new Date(cliente.created_at).toLocaleDateString("pt-BR")}</div>
        </div>
        <div className="md:col-span-2">
          <Label className="text-xs text-muted-foreground">Observações</Label>
          <div className="whitespace-pre-wrap">{cliente.observacoes || "—"}</div>
        </div>
      </CardContent>
    </Card>
  );
            ...
            {data.map((c: any) => (
              <TableRow key={c.id}>
                <TableCell>
                  ...
                  >
                    <Star
                      className={`h-4 w-4 ${c.principal ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`}
                    />
                  </Button>
                </TableCell>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {data.map((o: any) => (
              <TableRow key={o.id}>
                <TableCell>
                  <Link to="/os/$id" params={{ id: o.id }} className="text-accent hover:underline">
                    #{o.numero}
                  </Link>
                </TableCell>
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("pagamentos")
          .select("*, ordens_servico!inner(numero, titulo, cliente_id)")
          .eq("ordens_servico.cliente_id", clienteId)
      ).data ?? [],
  });
  return (
