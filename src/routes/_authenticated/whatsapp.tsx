...
const mensagensMock = [
  ...
  { id: 4, dir: "out", txt: "Perfeito. Posso preparar o orçamento agora. Tem prazo desejado?", hora: "10:18" },
    ...
    <div className="h-[calc(100vh-8rem)] grid grid-cols-12 gap-4">
          ...
          {conversasWhatsapp.map((c) => (
            ...
            >
              <div className="flex items-start gap-3">
                <Avatar className="h-9 w-9"><AvatarFallback>{c.nome[0]}</AvatarFallback></Avatar>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Badge variant="outline" className="text-[10px] py-0">{c.etiqueta}</Badge>
                    {c.naoLidas > 0 && (
                      <Badge className="text-[10px] py-0 bg-emerald-600 hover:bg-emerald-600">{c.naoLidas}</Badge>
                    )}
                  </div>
                </div>
              </div>
      ...
      <Card className="col-span-6 flex flex-col overflow-hidden">
        <div className="p-3 border-b flex items-center gap-3">
          <Avatar><AvatarFallback>{selected.nome[0]}</AvatarFallback></Avatar>
          <div>
          ...
          {mensagensMock.map((m) => (
            <div key={m.id} className={`flex ${m.dir === "out" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] rounded-lg p-2.5 text-sm ${
                ...
                <div>{m.txt}</div>
                <div className={`text-[10px] mt-1 ${m.dir === "out" ? "text-emerald-50" : "text-muted-foreground"}`}>{m.hora}</div>
              </div>
            </div>
          ))}
        </div>
        <div className="p-3 border-t flex items-center gap-2">
          ...
          <Button size="icon" className="bg-emerald-600 hover:bg-emerald-700"><Send className="h-4 w-4" /></Button>
        </div>
      </Card>
      ...
      <Card className="col-span-3 flex flex-col overflow-auto">
        <div className="p-4 border-b">
          ...
          <div className="flex items-center gap-2 font-medium"><User className="h-4 w-4" /> {selected.nome}</div>
        ...
        <div className="p-4 border-b space-y-2">
          ...
          <Button variant="outline" size="sm" className="w-full justify-start"><FileText className="h-4 w-4 mr-2" /> Criar orçamento</Button>
          <Button variant="outline" size="sm" className="w-full justify-start"><ClipboardList className="h-4 w-4 mr-2" /> Criar OS</Button>
          <Button variant="outline" size="sm" className="w-full justify-start"><Tag className="h-4 w-4 mr-2" /> Etiquetar</Button>
        </div>
        <div className="p-4 space-y-3">
          <div className="text-xs uppercase tracking-wider text-muted-foreground">Histórico</div>
          <div className="text-sm space-y-2">
            <div className="flex justify-between"><span>OS-1042</span><Badge variant="outline">Em produção</Badge></div>
            <div className="flex justify-between"><span>OS-1031</span><Badge variant="outline">Concluído</Badge></div>
            <div className="flex justify-between"><span>Orç #245</span><Badge variant="outline">Enviado</Badge></div>
          </div>
        </div>
      </Card>
    </div>
