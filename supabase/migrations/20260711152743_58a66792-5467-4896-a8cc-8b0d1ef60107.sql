CREATE TABLE IF NOT EXISTS public.permissoes (
  chave TEXT PRIMARY KEY, dominio TEXT NOT NULL, descricao TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS public.perfil_permissoes (
  perfil TEXT NOT NULL, permissao TEXT NOT NULL REFERENCES public.permissoes(chave) ON DELETE CASCADE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY (perfil, permissao)
);
GRANT SELECT ON public.permissoes, public.perfil_permissoes TO authenticated;
GRANT ALL ON public.permissoes, public.perfil_permissoes TO service_role;
ALTER TABLE public.permissoes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.perfil_permissoes ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "permissoes read all" ON public.permissoes;
CREATE POLICY "permissoes read all" ON public.permissoes FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "perfil_permissoes read all" ON public.perfil_permissoes;
CREATE POLICY "perfil_permissoes read all" ON public.perfil_permissoes FOR SELECT TO authenticated USING (true);
...
('orcamentos.read','orcamentos','Ler'),('orcamentos.create','orcamentos','Criar'),('orcamentos.update','orcamentos','Atualizar'),('orcamentos.send','orcamentos','Enviar'),('orcamentos.approve','orcamentos','Aprovar'),('orcamentos.cancel','orcamentos','Cancelar'),('orcamentos.convert','orcamentos','Converter'),('desconto.request','orcamentos','Solicitar desconto'),('desconto.approve','orcamentos','Aprovar desconto'),('margem.read','orcamentos','Ler margem'),
('os.read','os','Ler'),('os.create','os','Criar'),('os.update','os','Atualizar'),('os.assign','os','Atribuir'),('os.status.advance','os','Avançar'),('os.status.override','os','Forçar'),('os.close','os','Fechar'),
('financeiro.read','financeiro','Ler'),('financeiro.sensitive.read','financeiro','Ler sensível'),('pagamentos.create','financeiro','Criar'),('pagamentos.update','financeiro','Atualizar'),('pagamentos.confirm','financeiro','Confirmar'),('pagamentos.reverse','financeiro','Estornar'),('custos.read','financeiro','Ler custos'),('resultado.read','financeiro','Ler resultado'),
...
SELECT p.perfil, pe.chave FROM (VALUES ('administrador'),('admin')) AS p(perfil) CROSS JOIN public.permissoes pe
...
CREATE OR REPLACE FUNCTION public.normalize_phone(_phone TEXT) RETURNS TEXT LANGUAGE SQL IMMUTABLE AS $fn$ SELECT NULLIF(regexp_replace(coalesce(_phone,''), '\D', '', 'g'), '') $fn$;
CREATE OR REPLACE FUNCTION public.normalize_email(_email TEXT) RETURNS TEXT LANGUAGE SQL IMMUTABLE AS $fn$ SELECT NULLIF(lower(trim(coalesce(_email,''))), '') $fn$;
CREATE OR REPLACE FUNCTION public.normalize_document(_doc TEXT) RETURNS TEXT LANGUAGE SQL IMMUTABLE AS $fn$ SELECT NULLIF(regexp_replace(coalesce(_doc,''), '\D', '', 'g'), '') $fn$;
...
UPDATE public.leads SET telefone_original = COALESCE(telefone_original, telefone) WHERE telefone IS NOT NULL;
...
UPDATE public.clientes SET documento_normalizado = COALESCE(documento_normalizado, public.normalize_document(documento)) WHERE documento IS NOT NULL;
...
GRANT SELECT, INSERT ON public.eventos_negocio TO authenticated;
GRANT ALL ON public.eventos_negocio TO service_role;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.whatsapp_contatos, public.whatsapp_anexos, public.whatsapp_participantes, public.whatsapp_fila_envio, public.whatsapp_webhook_eventos TO authenticated;
GRANT ALL ON public.whatsapp_contatos, public.whatsapp_anexos, public.whatsapp_participantes, public.whatsapp_fila_envio, public.whatsapp_webhook_eventos TO service_role;
ALTER TABLE public.whatsapp_contatos ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_anexos ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_participantes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_fila_envio ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whatsapp_webhook_eventos ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "wa staff all" ON public.whatsapp_contatos;
CREATE POLICY "wa staff all" ON public.whatsapp_contatos FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
DROP POLICY IF EXISTS "wa staff all" ON public.whatsapp_anexos;
CREATE POLICY "wa staff all" ON public.whatsapp_anexos FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
DROP POLICY IF EXISTS "wa staff all" ON public.whatsapp_participantes;
CREATE POLICY "wa staff all" ON public.whatsapp_participantes FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
DROP POLICY IF EXISTS "wa staff all" ON public.whatsapp_fila_envio;
CREATE POLICY "wa staff all" ON public.whatsapp_fila_envio FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
DROP POLICY IF EXISTS "wa staff all" ON public.whatsapp_webhook_eventos;
CREATE POLICY "wa staff all" ON public.whatsapp_webhook_eventos FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.orcamento_versoes, public.orcamento_versao_itens TO authenticated;
GRANT ALL ON public.orcamento_versoes, public.orcamento_versao_itens TO service_role;
ALTER TABLE public.orcamento_versoes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orcamento_versao_itens ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "orc_versoes staff all" ON public.orcamento_versoes;
CREATE POLICY "orc_versoes staff all" ON public.orcamento_versoes FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
DROP POLICY IF EXISTS "orc_versoes_itens staff all" ON public.orcamento_versao_itens;
CREATE POLICY "orc_versoes_itens staff all" ON public.orcamento_versao_itens FOR ALL TO authenticated USING (is_staff(auth.uid())) WITH CHECK (is_staff(auth.uid()));
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.contas_receber, public.parcelas_receber TO authenticated;
GRANT ALL ON public.contas_receber, public.parcelas_receber TO service_role;
ALTER TABLE public.contas_receber ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.parcelas_receber ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "contas_receber financeiro all" ON public.contas_receber;
CREATE POLICY "contas_receber financeiro all" ON public.contas_receber FOR ALL TO authenticated USING (public.has_permission(auth.uid(),'financeiro.read')) WITH CHECK (public.has_permission(auth.uid(),'financeiro.read'));
DROP POLICY IF EXISTS "parcelas_receber financeiro all" ON public.parcelas_receber;
CREATE POLICY "parcelas_receber financeiro all" ON public.parcelas_receber FOR ALL TO authenticated USING (public.has_permission(auth.uid(),'financeiro.read')) WITH CHECK (public.has_permission(auth.uid(),'financeiro.read'));
...
BEGIN
  ...
  IF v_existing IS NULL THEN
    ...
    VALUES (COALESCE(p_dados->>'nome', v_lead.nome), COALESCE(p_dados->>'empresa', v_lead.empresa), COALESCE(p_dados->>'documento', v_lead.documento), public.normalize_email(COALESCE(p_dados->>'email', v_lead.email)), COALESCE(p_dados->>'telefone', v_lead.telefone_original), v_lead.responsavel_id, v_uid, 'Criado por conversão de lead') RETURNING id INTO v_cliente_id;
...
BEGIN
  ...
  IF v_pag.parcela_id IS NOT NULL THEN UPDATE public.parcelas_receber SET status='prevista' WHERE id=v_pag.parcela_id; END IF;
