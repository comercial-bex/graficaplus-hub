-- Parte 2: operação completa da OS (tarefas, arquivos, estoque, agenda, produção, qualidade, logística e fechamento operacional).
...
-- -----------------------------------------------------------------------------
-- Planejamento por item e sugestões congeláveis de produto
-- -----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_os_tarefas_os ON public.os_tarefas(os_id, status, prazo);
CREATE INDEX IF NOT EXISTS idx_os_tarefas_resp ON public.os_tarefas(responsavel_id, status);
...
-- -----------------------------------------------------------------------------
-- Arquivos/versionamento/aprovação/token externo
-- -----------------------------------------------------------------------------
...
ALTER TABLE public.arquivos ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'rascunho';
ALTER TABLE public.arquivos ADD COLUMN IF NOT EXISTS final BOOLEAN NOT NULL DEFAULT false;
CREATE UNIQUE INDEX IF NOT EXISTS uq_arquivos_final_escopo ON public.arquivos(COALESCE(os_id,'00000000-0000-0000-0000-000000000000'::uuid), COALESCE(os_item_id,'00000000-0000-0000-0000-000000000000'::uuid), COALESCE(tipo,'')) WHERE final;
...
INSERT INTO public.material_lotes(material_id, codigo, quantidade, unidade, custo_unitario_snapshot, fornecedor)
SELECT m.id, 'LEGADO', COALESCE(m.estoque,0), COALESCE(m.unidade,'un'), COALESCE(m.custo_unitario, m.custo_medio, 0), m.fornecedor
FROM public.materiais m
WHERE COALESCE(m.estoque,0) > 0 AND NOT EXISTS (SELECT 1 FROM public.material_lotes l WHERE l.material_id=m.id);
...
ALTER TABLE public.movimentacoes_estoque ADD COLUMN IF NOT EXISTS usuario_id UUID REFERENCES public.usuarios(id) ON DELETE SET NULL;
ALTER TABLE public.movimentacoes_estoque ADD COLUMN IF NOT EXISTS unidade TEXT NOT NULL DEFAULT 'un';
ALTER TABLE public.movimentacoes_estoque ADD COLUMN IF NOT EXISTS origem TEXT NOT NULL DEFAULT 'manual';
...
-- -----------------------------------------------------------------------------
-- Agenda canônica, manutenção e produção
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Qualidade, ocorrências/retrabalho, logística, custos
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  FOR r IN SELECT * FROM public.os_materiais_previstos WHERE os_id=p_os_id LOOP
    ...
    FOR l IN SELECT * FROM public.material_lotes WHERE material_id=r.material_id AND (quantidade-quantidade_reservada)>0 ORDER BY validade NULLS LAST, created_at FOR UPDATE LOOP
      ...
      INSERT INTO public.estoque_reservas(os_id, os_item_id, tarefa_id, material_id, lote_id, quantidade, status, created_by) VALUES (p_os_id, r.os_item_id, r.tarefa_id, r.material_id, l.id, v_reservado, CASE WHEN v_reservado<r.quantidade THEN 'parcial' ELSE 'reservada' END, v_uid) ON CONFLICT DO NOTHING;
...
BEGIN
  ...
  FOR r IN SELECT * FROM public.estoque_reservas WHERE os_id=p_os_id AND status IN ('reservada','parcial') FOR UPDATE LOOP
    ...
    SELECT * INTO l FROM public.material_lotes WHERE id=r.lote_id FOR UPDATE; IF NOT FOUND OR (l.quantidade-l.quantidade_reservada) + r.quantidade < v_qtd THEN RAISE EXCEPTION 'Saldo insuficiente no lote %', r.lote_id; END IF;
  ...
  END LOOP;
  IF p_consumos IS NOT NULL AND jsonb_array_length(v_movs)=0 THEN
    FOR r IN SELECT * FROM jsonb_to_recordset(p_consumos) AS x(material_id UUID, os_item_id UUID, tarefa_id UUID, quantidade NUMERIC) LOOP
      v_qtd := r.quantidade;
      FOR l IN SELECT * FROM public.material_lotes WHERE material_id=r.material_id AND quantidade>0 ORDER BY validade NULLS LAST, created_at FOR UPDATE LOOP
        EXIT WHEN v_qtd <= 0;
        IF l.quantidade <= 0 THEN CONTINUE; END IF;
        UPDATE public.material_lotes SET quantidade=quantidade-LEAST(v_qtd,l.quantidade) WHERE id=l.id;
        INSERT INTO public.movimentacoes_estoque(material_id,lote_id,tipo,quantidade,unidade,custo_unitario_snapshot,os_id,os_item_id,tarefa_id,usuario_id,origem,motivo) VALUES (r.material_id,l.id,'saida',LEAST(v_qtd,l.quantidade),l.unidade,l.custo_unitario_snapshot,p_os_id,r.os_item_id,r.tarefa_id,v_uid,'baixa_os','Baixa transacional informada') RETURNING id INTO v_mov;
        INSERT INTO public.custos_operacionais_os(os_id,os_item_id,tarefa_id,categoria,origem,quantidade,valor_unitario,usuario_id) VALUES (p_os_id,r.os_item_id,r.tarefa_id,'material','baixa_estoque',LEAST(v_qtd,l.quantidade),l.custo_unitario_snapshot,v_uid);
        v_movs := v_movs || jsonb_build_object('movimentacao_id',v_mov,'material_id',r.material_id,'quantidade',LEAST(v_qtd,l.quantidade));
        v_qtd := v_qtd - LEAST(v_qtd,l.quantidade);
      END LOOP;
      IF v_qtd > 0 THEN RAISE EXCEPTION 'Saldo insuficiente para material %', r.material_id; END IF;
    END LOOP;
  END IF;
...
-- RLS básico por permissões
DO $$ DECLARE t TEXT; BEGIN
  FOREACH t IN ARRAY ARRAY['os_tarefas','os_tarefa_comentarios','arquivo_aprovacoes','arquivo_tokens_externos','material_lotes','os_materiais_previstos','estoque_reservas','estoque_inventarios','maquinas_agenda','maquina_compatibilidades','apontamentos_producao','qualidade_checklists','qualidade_inspecoes','custos_operacionais_os','notificacoes_operacionais'] LOOP
    EXECUTE format('ALTER TABLE public.%I ENABLE ROW LEVEL SECURITY', t);
  END LOOP;
END $$;

-- Policies permissivas para service/staff, leitura/escrita controlada pelas RPCs e permissões de domínio.
...
DROP POLICY IF EXISTS "agenda read" ON public.maquinas_agenda;
CREATE POLICY "agenda read" ON public.maquinas_agenda FOR SELECT TO authenticated USING (public.has_permission(auth.uid(),'agenda.read'));
DROP POLICY IF EXISTS "agenda write" ON public.maquinas_agenda;
CREATE POLICY "agenda write" ON public.maquinas_agenda FOR ALL TO authenticated USING (public.has_permission(auth.uid(),'agenda.schedule') OR public.has_permission(auth.uid(),'agenda.reschedule')) WITH CHECK (public.has_permission(auth.uid(),'agenda.schedule') OR public.has_permission(auth.uid(),'agenda.reschedule'));
